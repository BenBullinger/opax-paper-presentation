...
def experiment(logs_dir: str, use_wandb: bool, time_limit: int, n_envs: int, exp_name: str,
    ...
    reward_model_to_goal = SwimmerRewardModel(distance_weight=1.0)
    reward_model_away_from_goal = SwimmerRewardModel(distance_weight=-1.0)
    env_kwargs_to_goal = {
        'reward_model': reward_model_to_goal,
        'render_mode': 'rgb_array'
    }

    env_kwargs_away_from_goal = {
        'reward_model': reward_model_away_from_goal,
        'render_mode': 'rgb_array'
    }
    ...
    test_env_to = make_vec_env(SwimmerEnvDM, wrapper_class=wrapper_cls_test, seed=seed,
                            env_kwargs=env_kwargs_to_goal, n_envs=1)
    test_env_away = make_vec_env(SwimmerEnvDM, wrapper_class=wrapper_cls_test, seed=seed,
                                 env_kwargs=env_kwargs_away_from_goal, n_envs=1)
    test_env = [test_env_to, test_env_away]
    ...
    optimizer_kwargs = {
        ...
        'transitions_per_update': 10000,
        'sac_kwargs': sac_kwargs,
        'sim_transitions_ratio': 0.25,
    ...
    if exploration_strategy == 'PETS':
        dynamics_model_to = ActiveLearningPETSModel(
        ...
        if exploration_strategy == 'HUCRL':
            ...
            dynamics_model_away = HUCRLModel(
                action_space=env.action_space,
                observation_space=env.observation_space,
                num_ensemble=num_ensembles,
                reward_model=reward_model_away_from_goal,
            ...
            dynamics_model_to = ActiveLearningHUCRLModel(
                action_space=env.action_space,
                observation_space=env.observation_space,
                num_ensemble=num_ensembles,
                reward_model=reward_model_to_goal,
