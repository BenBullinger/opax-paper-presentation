...
class ModelBasedAgent(DummyAgent):
    ...
    exploration_strategy: str, exploration strategy ('Optimistic' for OpAx, 'OpRX' for OpRX)
    """
    def __init__(
            self,
            action_space: gym.spaces.box,
        ...
        # Set OpRX mode flag on the optimizer if using OpRX
        if self.exploration_strategy == 'OpRX' and hasattr(self.policy_optimizer, '_set_oprx_mode'):
            self.policy_optimizer._set_oprx_mode(True)
        elif self.exploration_strategy == 'ORX' and hasattr(self.policy_optimizer, '_set_orx_mode'):
    ...
    def act_in_jax(self, obs: jax.Array, rng: jax.random.PRNGKeyArray, eval: bool = False, eval_idx: int = 0):
        ...
        if isinstance(self.policy_optimizer, SACOptimizer):
            ...
            # ORX uses the default path (same as OpAx) since it still does trajectory optimization
            if self.exploration_strategy == 'OpRX':
                # OpRX: Use regular trajectory optimization but with random action sequences
                # This ensures exact compatibility with OpAx's data flow
                if eval:
                ...
                action = action_sequence[:, 0, ...]
            # Slice actions for all cases since both OpAx and OpRX now return [action, eta] format
            action = action[..., :self.action_space.shape[0]]
        return action
