...
class ICemTO(DummyPolicyOptimizer):
    def __init__(self,
        ...
        self._oprx_mode = False  # Flag for OpRX random sampling mode
    ...
    def _optimize_with_params(
        ...
        if self._oprx_mode:
            # OpRX mode: Return random action sequence using colored noise with exponent
            if optimizer_key is None:
                optimizer_key = jax.random.PRNGKey(self.seed)
            
            # Generate random action sequence using colored noise (same as OpAx but no optimization)
            opt_size = self.opt_dim[0] * self.opt_dim[1]
            colored_noise = powerlaw_psd_gaussian(
                exponent=self.opt_params.exponent, 
                size=opt_size, 
                rng=optimizer_key
            )
            random_sequence = colored_noise.reshape(self.opt_dim)
            random_sequence = jnp.clip(random_sequence, a_max=1, a_min=-1)
            
            # Evaluate the random sequence to get a reward (for consistency)
            ...
            def get_reward(obs, eval_key):
                transition = eval_func(random_sequence, obs, eval_key)
                return transition.reward.mean()
            
            if key is not None:
                eval_keys = jax.random.split(key=key, num=self.n_particles)
                rewards = jax.vmap(get_reward)(initial_state, eval_keys)
            else:
                rewards = jax.vmap(get_reward, in_axes=(0, None))(initial_state, key)
            
            avg_reward = rewards.mean()
            return random_sequence, avg_reward
        
        # Original OpAx optimization code
        initial_state = jnp.repeat(jnp.expand_dims(initial_state, 0), self.n_particles, 0)
        eval_func = lambda seq, x, k: sample_trajectories(
            evaluate_fn=evaluate_fn,
            parameters=dynamics_params,
    ...
    def _set_oprx_mode(self, mode: bool):
        """Set OpRX mode for random sampling instead of optimization."""
        self._oprx_mode = mode
